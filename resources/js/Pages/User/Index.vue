<template>
  <app-layout>
    ...
    <template #header>
      <h1 class="text-white font-semibold text-lg">Users Management</h1>
    </template>
            ...
            <div class="relative flex flex-col min-w-0 break-words w-full mb-6  rounded" :class="[color === 'light' ? 'bg-white' : 'bg-emerald-900 text-white']">
                ...
                <div class="flex flex-wrap items-center">
                  <div class="relative w-full px-4 max-w-full flex-grow flex-1">
                    <div class="text-center flex justify-between items-center">
                      ...
                      <inertia-link :href="route('user.create')" class="bg-emerald-500 text-white active:bg-emerald-600 font-bold uppercase text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none mr-1 ease-linear transition-all duration-150" type="button">
                        Add new
                      </inertia-link>
                    </div>
                  </div>
              ...
              <div class="block w-full overflow-x-auto shadow-md">
                ...
                <table class="items-center w-full bg-transparent border-collapse">
                  ...
                  <tbody>
                    <tr v-for="(item, key) in users.data" :key="key" class="text-blueGray-500">
                      ...
                      <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-left flex items-center">
                        <div class="inline-flex w-10 h-10 mr-2">
                          <img class='w-10 h-10 object-cover rounded-full border-2 border-blueGray-300' alt='User avatar' :src="item.profile_photo_url">
                        </div>
                        <div>
                          <p class="text-blueGray-700 text-sm">
                            {{ item.name }}
                          </p>
                          <p class="text-blueGray-500 text-sm tracking-wide">
                            {{ item.email }}
                          </p>
                        </div>
                      </td>
  ...
  watch: {
    params: {
      handler: throttle(function () {
        ...
        this.$inertia.get(this.route("user.index"), params, {
          replace: true,
          preserveState: true,
        });
      }, 150),
