<template>
  <app-layout>
    ...
    <template #header>
      <h1 class="text-white font-semibold text-lg">Edit User {{user.data.name}}</h1>
    </template>
                  ...
                  <div class="flex flex-wrap">
                    <div class="w-full px-4">
                      <div class="relative w-full mb-3">
                        ...
                        <multiselect v-model="form.roles" deselect-label="Can't remove this value" track-by="name" label="name" placeholder="Select one" :options="options" :searchable="true" :allow-empty="false">
...
export default {
  ...
  methods: {
    store() {
      this.form.put(this.route("user.update", this.user.data.id), {
