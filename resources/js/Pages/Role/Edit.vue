<template>
  <app-layout>
    ...
    <template #header>
      <h1 class="text-white font-semibold text-lg">Edit role {{ role.data.name }} </h1>
    </template>
            ...
            <div class="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded-lg bg-blueGray-100 border-0">
              <div class="rounded-t bg-white mb-0 px-6 py-6">
                <div class="text-center flex justify-end">
                  <jet-button @click.native="update" class="bg-emerald-500 text-white active:bg-emerald-600 font-bold uppercase text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none mr-1 ease-linear transition-all duration-150" type="submit">
                    Update
                  </jet-button>
                </div>
              </div>
                  ...
                  <div class="flex flex-wrap">
                    ...
                    <div class="w-full lg:w-6/12 px-4">
                      <div class="relative w-full mb-3">
                        ...
                        <input type="text" class="border-0 px-3 py-3 placeholder-blueGray-300 text-blueGray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150" :value="slug" readonly />
                      </div>
                    </div>
                  </div>
                        ...
                        <ul class="flex flex-wrap">
                          <li v-for="permission in permissions" :key="permission.id">
                            <div class="inline-flex items-center">
                              ...
                              <div @click="deletePermission(permission.name)">
                                <svg class="mr-2 w-4 h-4 hover:text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
...
export default {
  ...
  data() {
    return {
      form: this.$inertia.form({
        ...
        permissions: this.role.data.permissions,
      }),
  ...
  methods: {
    update() {
      this.form.put(this.route("role.update", this.role.data.id), {
    ...
    deletePermission(permission) {
      // if (
      //   confirm(
      //     "Are you sure you want to delete this " + permission + " permission?"
      //   )
      // ) {
      //   this.$inertia.delete(this.route("permission.destroy", permission));
      // }
      alert(
        "Are you sure you want to delete this " + permission + " permission?"
      );
